...
const LineTooltip = ({ active, payload, label }) => {
  ...
  return (
    <div style={{
      ...
      <p style={{ fontSize: 11, color: "#94a3b8", marginTop: 2 }}>Hostels Opted</p>
    </div>
  );
};
...
const PieTooltip = ({ active, payload }) => {
  ...
  return (
    <div style={{
      ...
      <p style={{ fontSize: 11, color: "#94a3b8" }}>{((payload[0].value / 3) * 100).toFixed(1)}% of total</p>
    </div>
  );
};
  ...
  return (
    <>
      <style>{`
        ...
        .h-anim-1 { opacity:0; animation: fadeSlideUp 0.4s ease 0.04s forwards; }
        .h-anim-2 { opacity:0; animation: fadeSlideUp 0.4s ease 0.10s forwards; }
        .h-anim-3 { opacity:0; animation: fadeSlideUp 0.4s ease 0.16s forwards; }
        .h-anim-4 { opacity:0; animation: fadeSlideUp 0.4s ease 0.22s forwards; }
        .h-anim-5 { opacity:0; animation: fadeSlideUp 0.4s ease 0.28s forwards; }
        .h-anim-6 { opacity:0; animation: fadeSlideUp 0.4s ease 0.34s forwards; }
        ...
        .hero-card:hover        { border-color: #c7d2fe !important; box-shadow: 0 4px 24px rgba(99,102,241,0.14) !important; transform: translateY(-1px); }

        .bar-fill { transition: width 1.2s cubic-bezier(.4,0,.2,1); }

        .growth-row { transition: background 0.15s ease; cursor: default; }
        ...
        .hostel-detail-row { transition: background 0.15s ease; cursor: default; }
        .hostel-detail-row:hover { background: #f8f9ff; }
        .hostel-detail-row:hover td:first-child { color: #4f46e5; }

        .occupancy-bar { transition: width 1.2s cubic-bezier(.4,0,.2,1); }
      `}</style>

      <div className="space-y-6" style={{ fontFamily: "'Plus Jakarta Sans', sans-serif" }}>

        {/* ── Page title ──────────────────────────────────────────── */}
        <div className="h-anim-1">
        ...
        {/* ── Stat cards ──────────────────────────────────────────── */}
        <div className="grid grid-cols-2 xl:grid-cols-4 gap-4 h-anim-2">
          ...
          <div className="col-span-2 xl:col-span-1 hero-card section-card px-5 py-5 flex items-center gap-4 cursor-default">
            ...
            <div>
              <p className="text-[30px] font-bold text-slate-800 leading-none">5</p>
              <p className="text-[12px] text-slate-400 font-medium mt-1">Total Hostels Opted</p>
              <div className="flex items-center gap-1 mt-1.5">
                <TrendingUp size={11} className="text-emerald-500" />
                <span className="text-[11px] font-semibold text-emerald-600">+1 this semester</span>
              </div>
            </div>
          </div>
            ...
            return (
              <div key={r.label} className={`section-card role-card-${r.color} px-4 py-4 cursor-default`}>
                ...
                <div className="mt-3 h-1.5 bg-slate-100 rounded-full overflow-hidden">
                  <div className="h-full rounded-full bar-fill" style={{ width: `${r.pctNum}%`, background: C.bar }} />
                </div>
              </div>
            );
          })}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-5 gap-5">
          ...
          {/* Line chart */}
          <div className="lg:col-span-3 chart-card p-5 md:p-6 h-anim-3">
            <div className="flex items-start justify-between mb-5">
              <div>
                <h3 className="text-[15px] font-bold text-slate-800 tracking-tight">Hostels Opted Over Time</h3>
                <p className="text-[12px] text-slate-400 font-medium mt-0.5">Cumulative hostels added since December</p>
              </div>
            ...
            <ResponsiveContainer width="100%" height={250}>
              <LineChart data={hostelsOverTime} margin={{ top: 4, right: 8, left: -20, bottom: 0 }}>
                ...
                <YAxis
                  ...
                  allowDecimals={false} domain={[0, 4]}
                />
          ...
          {/* Pie chart */}
          <div className="lg:col-span-2 chart-card p-5 md:p-6 h-anim-4 flex flex-col">
            <div className="mb-2">
              <h3 className="text-[15px] font-bold text-slate-800 tracking-tight">Hostels by Type</h3>
              <p className="text-[12px] text-slate-400 font-medium mt-0.5">Distribution by gender type</p>
            </div>
                ...
                <div key={r.name} className="pie-row flex items-center justify-between gap-3">
                  ...
                  <div className="flex items-center gap-2 flex-shrink-0">
                    <div className="w-20 h-1.5 bg-slate-100 rounded-full overflow-hidden">
                      <div className="h-full rounded-full bar-fill" style={{ width: `${(r.value / 3) * 100}%`, background: r.color }} />
                    </div>
                    <span className="text-[12px] font-bold text-slate-700 w-4 text-right">{r.value}</span>
                  </div>
                </div>
              ))}
            </div>
        ...
        {/* ── Monthly opted table ─────────────────────────────────── */}
        <div className="chart-card p-5 md:p-6 h-anim-6">
          <div className="mb-5">
            <h3 className="text-[15px] font-bold text-slate-800 tracking-tight">Monthly Growth</h3>
            <p className="text-[12px] text-slate-400 font-medium mt-0.5">Hostels opted each month</p>
          </div>
          <div className="overflow-x-auto">
            <table className="w-full" style={{ fontFamily: "'Plus Jakarta Sans', sans-serif" }}>
              <thead>
                <tr className="border-b border-slate-100">
                  {["Month", "Added", "Cumulative", "Status"].map(h => (
                    <th key={h} className="text-left text-[11px] font-bold text-slate-400 uppercase tracking-widest pb-3 pr-6 last:pr-0">{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                      ...
                      <td className="py-3 pr-6">
                        <div className="flex items-center gap-3">
                          <div className="w-24 h-1.5 bg-slate-100 rounded-full overflow-hidden">
                            <div className="h-full bg-indigo-500 rounded-full bar-fill" style={{ width: `${(added / 1) * 100}%` }} />
                          </div>
